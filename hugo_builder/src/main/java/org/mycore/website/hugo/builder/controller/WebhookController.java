...
//	private static String SIGNATURE = "5401cf99215131b5f92af44684ba1d15ea5f80d7";
	...
	private static List<Status> STATUS_HISTORY = new ArrayList<>();
	@Autowired
			...
			if ("push".equals(gitEvent) && userAgent.startsWith("GitHub-Hookshot/") && branch.equals(ref)) {
				...
				if (validate(secret, json, sig)) {
					...
					for (JsonElement j : jsonData.get("commits").getAsJsonArray()) {
						status.getCommits().add(new GitCommit(j.getAsJsonObject()));
					}
					Status result = new Status();
					result.setSuccessful(status.isSuccessful());
					result.setCompleted(status.getCompleted());
					result.getErrors().addAll(status.getErrors());
	...
	@GetMapping("/run")
	public Status run() {
		STATUS_HISTORY.add(0, hugobuilderService.run());
		while (STATUS_HISTORY.size() > 10) {
			STATUS_HISTORY.remove(STATUS_HISTORY.size() - 1);
		}
